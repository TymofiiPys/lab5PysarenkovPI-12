...
void dictionary::dict::output(Word* n) {
	if (!n)
		return;
	output(n->left);
	std::cout << "Слово: " << n->eng << "\n\t- Укр. переклад: " << n->ukr << "\n\t- Звернень: " << std::to_string(n->uses) << "\n\n";
	output(n->right);
}
...
void dictionary::dict::createNewTree()//Створення дерева, сформованого за кількістю показників
{
	...
	while (true) {
		...
		if (!Top1)
			...
			add(Top1, new Word(w->eng, w->ukr, w->uses));//Вносимо в список слово із тими ж параметрами, але без "дітей"
		changeMax(TopCopy, w);
	}	
